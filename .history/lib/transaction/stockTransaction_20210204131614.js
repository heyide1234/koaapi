...
async function RefreshKC() {
    ...
    console.log("len=====", dt.length)
    // console.log("len=====", dt)
    for (let i = 0; i < dt.length; i++) {
        let dts = await apiModel.find({ table: "stock", where: { MaterialNumber: dt[i].MateriNumber } });
        if (dts.length == 0) {
            console.log("-----", dt[i].MaterialNumber)
            await apiModel.insert({
                table: 'stock', form: {
                    MaterialNumber: dt[i].MateriNumber,//物料编号
                    MaterialName: dt[i].MateriNumber,//物料名称
                    MaterialSpec: dt[i].MateriNumber,//规格型号
                    Thumbnail: dt[i].MateriNumber,//缩略图
                    Number: "0",//数量
                    SYNumber: "0",//剩余数
                    Location: "",//库位
                    MaterialSource: "",//材料来源
