...
async function goodsWarehousing(row, vd, datatemp, creater, creatdate) {
    ...
    //增加库存
    insertStore(row, creater, creatdate);
    //修改SN状态 status=3
    // updateSat(row.MaterialNumber);
    ...
    datatemp.filter((item) => {
        if (item.MaterialNumber === row.MaterialNumber) {
            ...
            await apiModel.updateByWhere({ table: "ManufacturingExecution", where: { SN: item.SN, }, form: { IsDeliverGoods: "1" } });
...
//增加库存
async function insertStore(row, creater, creatdate) {
    let res = await apiModel.find({ table: "stock", where: { MaterialNumber: row.MaterialNumber } });
    if (res.length == 0) {
        let v = {
            MaterialNumber: row.MaterialNumber, //物料编号
