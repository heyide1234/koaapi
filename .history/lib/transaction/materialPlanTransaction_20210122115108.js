...
async function addMeterial(datas, orderNumber) {
    for (let i = 0; i < datas.length; i++) {
        delete datas[i]._id;
        datas[i].status = "0";
        await apiModel.insert({ table: 'MatersialDemand', form: datas[i] });
    }
    console.log("orderNumber==", "920210121002" == orderNumber)
    ///进入推单模式
    apiModel.update({ table: 'salesOrder', where: { OrderNumber: orderNumber }, form: { processCode: "3" } });
}
////////////////
/////工具函数//////
///////////////
