...
async function JSMeterial(datast, orderNumber) {
    ...
    for (let i = 0; i < datast.length; i++) {
        ...
        let pynum = await goYYB(newrow.MaterialNumber); //该物料盘盈数
        let num = pynum - parseInt(newrow.Number); //盘盈数-需求数
        if (num >= 0) {
            //盘盈数>=需求数
            //1.修改盘盈数=num
            updateYYB(newrow.MaterialNumber, num + "");
            //2.修改采购值
            newrow.Number = "0"; //
        } else {
            //盘盈数<需求数
            //1.修改盘盈数=0
            updateYYB(newrow.MaterialNumber, "0");
            //2.修改采购值
...
async function goYYB(materialNumber) {
    await apiModel.find({ table: "YYB", where: { MaterialNumber: materialNumber } });
