...
async function getProcessState(orderNumber, processCode) {
    ...
    // apiModel.update({ table: "PurchaseOrder", id: row._id, form: fs });
    // apiModel.updateByWhere({ table: "salesOrder", where: { OrderNumber: tempsd.OrderNumber }, form: { processCode: "6" } });
    let res = await apiModel.find({ table: "salesOrder", where: { OrderNumber: orderNumber } });
    if (parseInt(res[0].processCode) < parseInt(processCode)) {
        apiModel.updateByWhere({ table: "salesOrder", where: { OrderNumber: orderNumber }, form: { processCode: processCode } });
