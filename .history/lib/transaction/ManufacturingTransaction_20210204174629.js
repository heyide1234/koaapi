...
function products(dts) {
    ...
    for (let m = 0; m < products.length; m++) {
        for (let n = 0; n < dts.length; n++) {
            if (products[m] === dts[n].MaterialNumber) {
                dd.push(dts[n])
                break;
            }
        }
    ...
    dts.filter(item => {
        if (products != {} && products.hasOwnProperty(item.MaterialNumber)) {
            console.log("...1..")
            delete products[item.MaterialNumber];
            dd.push(item)
