...
async function yyFind(data) {
    for (let i = 0; i < data.length; i++) {
        ...
        if (res.length > 0) {
            ...
            //添加为0的记录
            await apiModel.insert({ table: 'YYB', form: { MaterialNumber: productNumber, Number: "0" } });
        }
        ...
        apiModel.updateByWhere({ table: "YYB", where: { MaterialNumber: productNumber }, form: { Number: YYBnum + "" } });
