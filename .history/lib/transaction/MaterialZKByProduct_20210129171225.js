...
class MaterialZKByProduct {
    ...
    static async diagram(ctx) {
        ...
        while (true) {
            await sleep(500);
            console.log("----4070050107-----", dataList.length, len)
...
async function zkdiagram(MaterialNumber, n1 = 1) {
    ...
    //let mums = await this.findnum(MaterialNumber); //当前产品对应的物料list
    let mums = await apiModel.find({ table: "__materialDetails", dataBase: "base", where: { Parent: productNumber } })
