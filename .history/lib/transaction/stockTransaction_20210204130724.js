...
async function RefreshKC() {
    ...
    for (let i = 0; i < dt.length; i++) {
        let dts = await apiModel.find({ table: "stock", where: { MaterialNumber: dt[0].MateriNumber } });
        if (dts.length == 0) {
            console.log("-----", dt[0].MateriNumber)
            await apiModel.insert({
                table: 'stock', form: {
                    MaterialNumber: dt[0].MateriNumber,//物料编号
                    MaterialName: dt[0].MateriNumber,//物料名称
                    MaterialSpec: dt[0].MateriNumber,//规格型号
                    Thumbnail: dt[0].MateriNumber,//缩略图
                    Number: "0",//数量
                    SYNumber: "0",//剩余数
                    Location: dt[0].MateriNumber,//库位
                    MaterialSource: dt[0].MateriNumber,//材料来源
                    Purpose: dt[0].MateriNumber,//用途
