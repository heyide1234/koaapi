...
class Manufacturing {
    ...
    static async ManufacturingExecutionTransaction(ctx) {
        ...
        try {
            OrderNumber = data.OrderNumber;
            await ManufacturingExecution(data.datas, data.row, data.forms);
            status = true;
        } catch (err) {
            status = false;
            returnData = `[MongoDB] ERROR: ${err}`;
...
async function insertByManufactringExecutionDetail(SN, OrderNumber, Subsequence, SubseqMaterialNumber, creater, creatdate) {
    ...
    let d = await apiModel.find({ table: "__materialDetails", dataBase: "base", where: { Parent: SubseqMaterialNumber } });
    /////////////////
...
async function ManufacturingExecution(datas, row, forms) {
    ...
    if (flg) {
        apiModel.updateByWhere({ table: 'SubManufacturingExecution', where: { SN: datas[0].SN, SubseqMaterialNumber: datas[0].SubseqMaterialNumber }, form: { status: "1" } });
    }
    // apiModel.updateByWhere({ table: 'ManufacturingExecution', where: { SN: datas[0].SN }, form: { status: "1" } });
}
