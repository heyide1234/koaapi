...
class Manufacturing {
    ...
    static async ManufacturingPlanTransaction(ctx) {
        ...
        try {
            ...
            await ManufacturingPlan(data.row, data.saveNum, data.creater, data.creatdate)
...
////////////执行计划单//////////////
let productsh = null;
async function ManufacturingPlan(row, saveNum, creater, creatdate) {
    //生成单个产品物料明细模板
    ...
    let df = {
        ...
        MaterialName: scope.row.MaterialName, //产品名称
        Thumbnail: scope.row.Thumbnail, //
        MaterialSpec: scope.row.MaterialSpec, //
        Number: "1", //产品数量
        status: "0", //状态
        creater: creater, //创建人
