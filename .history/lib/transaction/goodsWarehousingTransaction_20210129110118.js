...
class goodsWarehousingTransaction {
    ...
    static async goodsWarehousingTransaction(ctx) {
        ...
        try {
            returndata = await goodsWarehousing(data.row, data.vd, data.datatemp, data.creater, data.creatdate);
        } catch (err) {
            console.log("err====", err)
            status = false;
    ...
    static async goodsDeliveryTransaction(ctx) {
        ...
        data = ctx.request.method.toLowerCase() == "get" ? ctx.request.query : ctx.request.body;
        try {
            returndata = await goodsDelivery(data.datas, data.seleArrT, data.Proportioner);
        } catch (err) {
            console.log("err====", err)
            status = false;
            returndata = `[MongoDB] ERROR: ${err}`;
...
async function goodsWarehousing(row, vd, datatemp, creater, creatdate) {
    //1.增加库存
    ...
    datatemp.filter((item) => {
        if (item.MaterialNumber === row.MaterialNumber) {
            apiModel.updateByWhere({ table: "ManufacturingExecution", where: { SN: item.SN, }, form: { IsDeliverGoods: "1" } });
        }
    });
    //4.添加到成品入库单记录
...
//成品出库
async function goodsDelivery(datas, seleArrT, Proportioner) {
    for (let j = 0; j < datas.length; j++) {
        let MaterialNumber = datas[j].MaterialNumber;
        let _Number = parseInt(datas[j].rknums); //int,需求数
        let res = await apiModel.find({ table: "stock", where: { MaterialNumber: MaterialNumber } });//库存查询
        apiModel.updateByWhere({ table: "stock", where: { MaterialNumber: MaterialNumber }, form: { Number: parseInt(res[0].Number) - _Number + "" } });
        //修改制造SN出货状态
        apiModel.updateByWhere({ table: "ManufacturingExecution", where: { SN: datas[i].SN }, form: { IsDeliverGoods: "2", } });
