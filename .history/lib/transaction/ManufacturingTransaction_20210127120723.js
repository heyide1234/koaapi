...
async function ManufacturingPlan(row, saveNum, LLNumber, creater, creatdate) {
    ...
    console.log('5...............', dat)
    if (dat.length > 0)
        apiModel.updateByWhere({ table: 'stock', where: { MaterialNumber: row.MaterialNumber }, form: { SYNumber: parseInt(dat[0].SYNumber) - parseInt(LLNumber) + "" } });
...
async function insertByManufactringExecutionDetail(SN, OrderNumber, creater) {
    ...
    for (let i = 0; i < d.length; i++) {
        ...
        apiModel.insert({ table: 'ManufacturingExecutionDetail', form: d[i] });
    }
}
////////////////
/////工具函数//////
