...
async function zkMeterial(product) {
    ...
    console.log("end!!!!!!!!!!!!!", dataList.length);
    ...
    for (let j = 0; j < dataList.length; j++) {
        ...
        if (dat.length > 0) {
            modelsCK[dataList[j].MaterialNumber] = dat[0].Number
        } else {
            modelsCK[dataList[j].MaterialNumber] = "0"
        }
    }
    console.log("物料清单===", models);
    for (const key in object) {
        let dat = await apiModel.find({ table: "stock", where: { MaterialNumber: object[key] } });
        if (dat.length > 0) {
            ck[dat[0]]
        }
    }

    //1.根据物料号计算出产品BOM dataBOM并且合并重复
    //   //2.循环Bom,仓库料号的数量
    //   for (let j = 0; j < dataBOM.length; j++) {
    //     // dataBOM[j].materialNumber//物料编码
    //     //3.根据物料编码算出仓库数并写入键值对
    //     ck[MaterialNumber] = ckNum;
    //   }
    //   var flag = true;
    //   var n = 0;
    //   while (flag) {
    //     n++;
    //     for (let m = 0; m < dataBOM.length; m++) {
    //       let nus = dataBOM[m].Number * n; //该物料需求数*循环次数
    //       if (ck[dataBOM[m].MaterialNumber] < nus) {
    //         flag = false;
    //         n--;
    //       }
    //     }
    //   }
    //   this.tableData[i].SYNumber = n + "";
    // }
