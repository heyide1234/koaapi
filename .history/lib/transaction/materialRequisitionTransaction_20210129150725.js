    ...
    for (let i = 0; i < datas.length; i++) {
        ...
        if (yyproductinfo.length > 0) {
            ...
            if (tempNum >= 0) {//库存足
                ...
                console.log("111", datas[i].Number);
            }
            await apiModel.updateByWhere({ table: "YYB", where: { MaterialNumber: datas[i].MaterialNumber }, form: { Number: YYBnum + "" } });
        }
        console.log("222", datas[i].Number);
        await apiModel.update({ table: "salesOrderDetail", id: datas[i]._id, form: { Number: datas[i].Number } });
        //2.添加到功能领料
        let dt = {
