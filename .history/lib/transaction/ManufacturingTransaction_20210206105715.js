...
async function productionModel(row) {
    ...
    let ffs = dtss.replace(/\[,{/g, "[{");
    console.log("产品BOM树控件。。。。。。。1=====", ffs)
    console.log("产品BOM树控件。。。。。。。2=====", JSON.parse(ffs))
...
async function ManufacturingExecution(datas, row, forms, creater, creatdate) {
    ...
    if (vfd) {
        apiModel.updateByWhere({ table: 'ManufacturingExecution', where: { SN: datas[0].SN }, form: { status: "1", creater: creater, creatdate: creatdate } });
    }
}
