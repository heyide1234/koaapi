...
async function products(dts) {
    ...
    for (let m = 0; m < products.length; m++) {
        let datas1 = await apiModel.find({ table: "__basicMaterialList", dataBase: "base", where: { MaterialNumber: products[m].MaterialNumber } });
        console.log("dddddd===", products[m].MaterialNumber, datas1)
        dd.push(datas1[0])
    }
