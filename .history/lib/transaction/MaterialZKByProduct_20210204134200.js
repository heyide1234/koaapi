...
//物料刷新
async function materialDetailsReflesh() {
    let dt = await apiModel.find({ table: "__materialDetails", dataBase: "base", where: {} });
    for (let i = 0; i < dt.length; i++) {
        let dts = await apiModel.find({ table: "__basicMaterialList", where: { MaterialNumber: dt[i].MaterialNumber } });
        if (dts.length > 0) {
            await apiModel.updateByWhere({
                table: "__materialDetails", where: { MaterialNumber: dt[i].MaterialNumber }, form: {
